...
const entryScreenStorageKey = 'scj-portfolio-entry-seen';
...
if (entryScreen) {
  ...
  const shouldPlayEntryScreen = (() => {
    if (prefersReducedEntryMotion.matches) {
      return false;
    }

    try {
      return !window.sessionStorage.getItem(entryScreenStorageKey);
    } catch {
      return true;
    }
  })();
  ...
  const entryExitDelay = shouldPlayEntryScreen ? 2200 : 0;
  const entryExitDuration = prefersReducedEntryMotion.matches ? 0 : 850;
  let isEntryClosed = !shouldPlayEntryScreen;
  ...
  if (!shouldPlayEntryScreen) {
    pageBody.classList.remove('is-intro-active');
    entryScreen.classList.add('is-hidden');
  } else {
    try {
      window.sessionStorage.setItem(entryScreenStorageKey, '1');
    } catch {
      // Continue normally when storage is unavailable. | EN: Continue normally when storage is unavailable.
    }
...
const supportsFineHeroMotion = window.matchMedia('(hover: hover) and (pointer: fine)');
...
if (heroSection) {
  ...
  const updateHeroPointerTarget = (event) => {
    if (!supportsFineHeroMotion.matches) {
      return;
    }
  ...
  const syncHeroMotionMode = () => {
    const canAnimateHero = !prefersReducedHeroMotion.matches
      && supportsFineHeroMotion.matches
      && heroMotion.isInView
      && !document.hidden;
  ...
  const heroVisibilityObserver = typeof IntersectionObserver === 'function'
    ? new IntersectionObserver((entries) => {
      heroMotion.isInView = entries.some((entry) => entry.isIntersecting);
      syncHeroMotionMode();
  ...
  heroSection.addEventListener('pointermove', updateHeroPointerTarget, { passive: true });
  ...
  document.addEventListener('visibilitychange', syncHeroMotionMode);
  ...
  if (typeof supportsFineHeroMotion.addEventListener === 'function') {
    supportsFineHeroMotion.addEventListener('change', syncHeroMotionMode);
  } else if (typeof supportsFineHeroMotion.addListener === 'function') {
    supportsFineHeroMotion.addListener(syncHeroMotionMode);
  }
...
if (aboutProfileImage) {
  ...
  const syncProfileFrameMotion = () => {
    const canAnimateProfileFrame = !prefersReducedProfileMotion.matches
      && profileFrameMotion.isInView
      && !document.hidden;
  ...
  const profileVisibilityObserver = typeof IntersectionObserver === 'function'
    ? new IntersectionObserver((entries) => {
      profileFrameMotion.isInView = entries.some((entry) => entry.isIntersecting);
      syncProfileFrameMotion();
    }, {
      threshold: [0, 0.35]
    })
  ...
  profileVisibilityObserver?.observe(aboutProfileImage);
...
const supportsFineButtonMotion = window.matchMedia('(hover: hover) and (pointer: fine)');
...
if (orbitButtonElements.length) {
  ...
  const syncOrbitButtonMotion = () => {
    const canAnimateOrbitButtons = !prefersReducedButtonMotion.matches
      && supportsFineButtonMotion.matches
      && !document.hidden;
  ...
  if (typeof supportsFineButtonMotion.addEventListener === 'function') {
    supportsFineButtonMotion.addEventListener('change', syncOrbitButtonMotion);
  } else if (typeof supportsFineButtonMotion.addListener === 'function') {
    supportsFineButtonMotion.addListener(syncOrbitButtonMotion);
  }
